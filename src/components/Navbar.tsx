        ...
        <div className="hidden md:flex items-center gap-8">
          {navItems.map((item) => (
            <Link
              key={item.id}
              href={item.href}
              className={`relative group text-sm font-medium transition-all duration-300 hover:text-primary-700 ${pathname === item.href ? "text-primary bg-gray-700/80 px-4 py-1 rounded-full" : "text-muted"}`}
            >
              {t(item.id)}
